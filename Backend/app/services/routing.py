...
def generate_route_comparison(
    ...
    beta_diagnostics = []
    safe_candidates = []
    requested_beta = beta if beta in BETA_VALUES else min(
        BETA_VALUES, key=lambda value: abs(value - beta)
    )
    for tested_beta in BETA_VALUES:
    ...
    requested_candidate = next(
        candidate for candidate in safe_candidates if candidate[0] == requested_beta
    )
    safe_beta, safe_route, reduction = requested_candidate
    meaningful = reduction >= 2.0 and safe_route["node_path"] != fast_route["node_path"]
    ...
    if not meaningful:
        alternatives = [
            ...
            and candidate[1]["distance_km"] <= fast_route["distance_km"] * 1.35
        ]
        if alternatives:
    ...
    if same_route or reduction < 2.0:
        message = (
        ...
        message = (
            f"La ruta segura reduce el riesgo acumulado en {reduction:.2f}% "
...
def _assign_segment_risks(
    ...
    raw_values = np.asarray([row["raw_risk"] for row in edge_rows], dtype=float)
    positive_values = raw_values[raw_values > 0]
    p95 = float(np.percentile(positive_values, 95)) if len(positive_values) else 1.0
    clipped = np.clip(raw_values, 0, p95)
    minimum = float(clipped.min()) if len(clipped) else 0.0
    maximum = float(clipped.max()) if len(clipped) else 1.0
    denominator = maximum - minimum

    for row, clipped_risk in zip(edge_rows, clipped):
        empirical = (
            float((clipped_risk - minimum) / denominator)
            if denominator > 1e-12
            else 0.0
        )
        predicted = risk_model.predict_point(
            row["midpoint"][0],
            row["midpoint"][1],
            modelo_riesgo=(
                "random_forest" if model_used == "Random Forest" else "xgboost"
            ),
        ).score
        normalized = min(1.0, max(0.0, 0.75 * empirical + 0.25 * predicted))
        data = row["data"]
        data.update(
            {
                "id_segmento": f"{row['u']}-{row['v']}-{row['key']}",
...
def _route_metrics(
    ...
    risk_values = [
        segment["riesgo_segmento_normalizado"] for segment in segments
    ]
    risk_total = float(sum(risk_values))
    risk_average = risk_total / len(risk_values) if risk_values else 0.0
    return {
        "route": [
